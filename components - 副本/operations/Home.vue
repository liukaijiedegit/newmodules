  ...
  <div class="mRight" id="menuBox">
    ...
    <div id="enterpriseEditModel">
      <el-dialog :close-on-click-modal="false" title="菜单 - 编辑" :visible.sync="dialogTableVisibleedit">
        <el-form :model="edit" ref="edit" label-width="30%" class="demo-ruleForm">
          <div class="formTable">
            <div class="block ">
              <el-form-item label="企业名称：" :rules="[{ required: true, message: ' '}]" prop="name">
                <el-input v-model="edit.name" maxlength="50" calss="corporate_name"></el-input>
              </el-form-item>
            </div>
              ...
              <div class="showtreeboxzTree" di="showtreeboxdisplaybox">
                <template>
                  <el-tabs v-model="activeCity" type="card" @tab-click="handleClickCity" witth="300">
                    ...
                    <el-tab-pane width="80" id="city1" height="300" label="市" name="city" :disabled=iscity>
                      <vue-scroll :ops="ops1" ref="vs">
                        <div class="TerminaLeft">
                    ...
                    <el-tab-pane width="80" id="county1" height="300" label="区" name="county" :disabled="icounty">
            ...
            <div class="block">
              <el-form-item label="联系人：" prop="parentname" :rules="[{ required: true, message: ' '}]">
                <el-input v-model="edit.contacts_names" maxlength="120"></el-input>
                <!-- <el-select v-model="add.parentname" @change="getvalue" placeholder="--请选择--">
                                    <el-option v-for="item in options" :key="item.index" :label="item.value" :value="item"></el-option>
                                </el-select> -->
              </el-form-item>
            ...
            <div class="block">
              <el-form-item label="联系人手机号：" :rules="[{ required: true, message: ' '}]" prop="url">
                <el-input v-model="edit.contacts_phonenums" maxlength="120"></el-input>
              </el-form-item>
            </div>
            <div class="power" @click="closedButton=false">
              <template>
                <el-tabs v-model="activeName" @tab-click="tabClick">
                  ...
                  <el-tab-pane :disabled="disabled" label="流媒体服务器" name="second">
                    <div class="second">
                      ...
                      <div id="medialist1">
                        ...
                        <ul>
                          <li @mousemove="loginglist" @mouseout="liimgshow" v-for="(item, index) in medialistedit"
                              :key="index">{{item.server_name}}
                            <span class="closebox"><img src="../../assets/close.png" alt=""
                                                        @click="imgclicking(item,index)"></span>
                          </li>
                  ...
                  <el-tab-pane :disabled="disabled" label="辅助服务器" name="third">
                    <span class="SelectTitle">选择辅助服务器：</span>
                    <div class="zSelect">
                      <el-select v-model="edit.assist_server.name" @change="getvalueedit" class="zgroup" placeholder="默认（本机">
                        <el-option v-for="item in options" :key="item.index" :label="item.name"
                                   :value="item"></el-option>
                      </el-select>
                    </div>
                      ...
                      <div class="scrolltree">
                        <vue-scroll :ops="ops1" ref="vs">
                          ...
                          <el-tree ref="tree_monitoredit" :props="props1" @node-expand="nodeclick3" lazy
                                   ...
                                   :load="loadNode3"
                                   node-key="area_code" show-checkbox></el-tree>
                        </vue-scroll>
                      </div>
                      ...
                      <div class="scrolltree">
                        <vue-scroll :ops="ops2" ref="vs">
                          <el-tree ref="tree_terminaledit" :props="props2"
                                   ...
                                   :load="loadNode4"
                                   node-key="region_code" show-checkbox></el-tree>
                        </vue-scroll>
                      </div>
          ...
          <div class="formTable">
            ...
            <div class="block">
              ...
              <el-form-item label="详情：" :rules="[{ required: true, message: ' '}]" prop="url">
                <el-input v-model="casuadata.密码" maxlength="120"></el-input>
              </el-form-item>
            </div>
...
//      this.servers()
    ...
    methods: {
      ...
      handleCheckedCitiesChange(val) {
        ...
        console.log(this.addenterprise.allocated_enterprise_functions)
      },
      //编辑时权限分配选中的值
      ...
      //注销企业操作
      unregister(index) {
        console.log(index) // var cheklength = this.multipleSelection;
        var _this = this
      ...
      Regiondetail() {
        ...
        getRegiondetail(Regiondetaillist).then(function (res) {
          ...
          if (res.status === 200 && res.data.result == "ok") {
            ...
            console.log(_this.addenterprise)
          ...
          getenterprise_info(enterprise_id).then(res => {
           ...
           this.editlist={
             allocated_enterprise_functions:res.data.data.allocated_enterprise_functions,
               ...
               enterprise_name: res.data.data.name,
               enterprise_streamservers:res.data.data.stream_servers,
               monitor_resource_organizations:res.data.data.monitor_resource_organizations,
              ...
              var functions=res.data.data.allocated_enterprise_functions
                 var ary = [];
                  ...
                  _this.all_functions.forEach(function (value, i) {
                    ...
                    //处理流媒体服务器
                     _this.medialistedit=res.data.data.stream_servers



                          console.log( _this.medialistedit)
           //处理辅助服务器
           ...
           monitorlist.forEach(function (value, i) {
             var ary = {area_code:value.region_code}

             _this.allmonitoreditId.push(ary)
           ...
           terminal_regionslist.forEach(function (value, i) {
             var ary = {region_code:value.region_code}

             _this.allterminalId.push(ary)
...
///监控资源分配新增开始19/7/11lkj
      ...
      loadNode1(node, resolve) {
        ...
        if (stringlist1 == '00' && this.is_all == false) {
          ...
          if (this.is_all != true) {
            ...
            params.area_code = node.data.area_code;

          }
          getresource_organizations(params)
            .then(function (res) {
              console.log(res)
              // _this.reLogin(res.data.code); //提示帐号登陆
              if (res.status == 200) {
      ...
      handleCheckChange1(data, checked, indeterminate) {
        ...
        if (checked===true) {
          ...
          getregion_paths(param).then(function (res) {
            // _this.reLogin(res.data.code); //提示帐号登陆、

            if (res.status === 200 && res.data.result == "ok") {

              console.log(res)
              serverlist.region_full_code=res.data.data.region_paths[0].ids
              serverlist.region_full_name=res.data.data.region_paths[0].names
...
//
      ...
      this.addenterprise.monitor_resource_organizations=regionlist
...
//获取终端通讯录地区目录/7/11lkj
      ...
      handleCheckChange2(data, checked, indeterminate) {
        ...
        var terminal_regions = {
          ...
          "region_full_code": "110000000000|110100000000|110102000000",
          "region_full_name": "北京市|北京市|西城区"
        }
...
//              _this.addenterprise.terminal_regions.push(terminal_regions)
        ...
        this.addenterprise.terminal_regions=terminallist
        ...
        if (checked) {
          getRegiondetail(llist).then(function (res) {
            ...
            if (res.status === 200 && res.data.result == "ok") {
              ...
              console.log(_this.addenterprise.terminal_regions);
            }
            if (res.data.result == "error") {
              _this.$message({
                message: res.data.error_description,
          ...
          _this.addenterprise.terminal_regions.forEach(function (value, i) {
            console.log(value);
            if (value.region_code === data.region_code)

              _this.addenterprise.terminal_regions.splice(1, i)
            console.log("删除后");
            console.log(_this.addenterprise.terminal_regions);
          })
...
<style type="text/css">
  ...
  #menucasualModel .el-checkbox,
  #enterpriseEditModel .el-checkbox,
